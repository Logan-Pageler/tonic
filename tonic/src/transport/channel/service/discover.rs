use crate::transport::channel::service::Connector;
...
use http::Uri;
use hyper::rt;
use hyper_util::client::legacy::connect::{HttpConnector, dns::GaiResolver};
        ...
        C::Response: rt::Read + rt::Write + Unpin + Send + 'static,
{
    pub(crate) fn new(changes: Receiver<Change<K, V>>) -> Self {
        Self { changes, _marker: PhantomData }
    }
}
        ...
        match Pin::new(&mut self.changes).poll_recv(cx) {
            ...
            Poll::Ready(Some(change)) => match change {
                Change::Insert(k, connection) => {
                    ...
                    Poll::Ready(Some(Ok(TowerChange::Insert(k, Connection::lazy(connector, endpoint)))))
                }
